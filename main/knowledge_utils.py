...
def upgrade_vul_to_device(ip: str, file_content: bytes, cookie: str = None, auto_get_cookie: bool = False) -> tuple:
    ...
    try:
        ...
        logger.info(f"上传漏洞库文件请求头: Cookie={cookie[:50]}...")
        ...
        logger.info(f"漏洞库升级请求参数: {json.dumps(data, ensure_ascii=False)}")
...
def upgrade_virus_to_device(ip: str, file_content: bytes, cookie: str = None, auto_get_cookie: bool = False) -> tuple:
    ...
    try:
        ...
        logger.info(f"上传病毒库文件请求头: Cookie={cookie[:50]}...")
        ...
        logger.info(f"病毒库升级请求参数: {json.dumps(data, ensure_ascii=False)}")
