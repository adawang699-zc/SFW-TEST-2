...
import threading
import queue
from asn1_decoder import SVDecoder
from network_utils import find_interface_by_name


class SVPacket(Packet):
    """SV 报文类"""
    name = "SV"
    fields_desc = []
...
class SVReceiverService:
    ...
    def _packet_handler(self, packet):
        ...
        try:
            ...
            # 检查以太网类型
            if packet[Ether].type != self.SV_ETHER_TYPE:
                return
            
            # 统计收到的SV报文数量
            self.sv_packet_count += 1
            
            # 创建简单的报文信息（不解析内容）
            packet_info = {
                'src_mac': packet[Ether].src,
                'dst_mac': packet[Ether].dst,
                'count': self.sv_packet_count
            }
    ...
    def _sniff_loop(self):
        ...
        try:
            ...
            # 过滤 SV 报文 (以太网类型 0x88BA)
            filter_str = f"ether proto {self.SV_ETHER_TYPE}"
