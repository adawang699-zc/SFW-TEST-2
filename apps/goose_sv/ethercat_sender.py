...
支持数据单元类型 1 (Ethercat DLPDU) 及多种命令码，启动 Agent 时需上传到 Agent 服务器。
"""
import struct
import time
import threading
...
class ECAT_Header(Packet):
    name = "EtherCAT Header (ETG标准+用户实测规则)"
    fields_desc = [
...
class ECAT_Datagram(Packet):
    name = "EtherCAT Datagram (ETG标准结构)"
    fields_desc = [
        ByteField("cmd", 0x01),
...
# 每个命令的默认参数 (dlen, send_data, adp, ado)
def _default_params(cmd_code, read_len=2):
    if cmd_code == 0x00:  # NOP
        return 0, b"", 0x0001, 0x0130
    if cmd_code in (0x01, 0x04, 0x08, 0x0A):  # 读命令
        dlen = read_len
        data = b""
    elif cmd_code in (0x0D, 0x0E, 0x0F):  # 读改写 4 字节
        dlen = 4
        data = b"\xFF\x00\x00\x55"
    elif cmd_code in (0x02, 0x09, 0x0B):  # 写
        dlen = 2
        data = b"\x11\x22" if cmd_code == 0x02 else (b"\x55\xAA" if cmd_code == 0x09 else b"\x99\x88")
    elif cmd_code in (0x03, 0x06, 0x0C):  # 读写
        dlen = 2
        data = b"\x33\x44" if cmd_code == 0x03 else (b"\x02\x03" if cmd_code == 0x06 else b"\x77\x66")
    elif cmd_code == 0x05:  # FPWR
        dlen = 2
        data = b"\x00\x01"
    elif cmd_code == 0x07:  # 保留
        dlen = 0
        data = b""
    else:
        dlen = read_len
