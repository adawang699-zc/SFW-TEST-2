...
支持帧类型、服务码、服务类型、Option/Suboption 可配置，按选定参数循环发送 DCP 报文。
"""
import struct
import time
import threading
...
OPTION_SUBOPTIONS = {
    ...
    0x07: [  # NME Domain
        (0x01, "NME Domain Name"),
        (0x02, "NME Manager"),
        (0x03, "NME Paramater UUID"),
        (0x04, "NME Agent"),
        (0x05, "CIM Interface"),
    ],
...
def build_and_send_dcp_request(iface, frame_id, service_id, service_type, option, suboption,
    ...
    """构造并发送单条 DCP 请求报文（仅 Option+Suboption 块，无 DCPBlockLength 的 Request）。"""
...
class DcpSenderService:
    """DCP 发送服务：按配置的帧类型、服务码、服务类型、Option、Suboption 循环发送。"""
    ...
    def _send_loop(self, frame_id, service_id, service_type, option_val):
        idx = 0
        subopts = list(self.suboptions)  # 固定副本，避免被修改
        n = len(subopts)
