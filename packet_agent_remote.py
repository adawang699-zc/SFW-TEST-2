...
def _get_client_manager(protocol: str):
    """获取指定协议的客户端管理器"""
    return _active_managers.get(protocol)

def _set_client_manager(protocol: str, manager):
    """设置客户端管理器"""
    _active_managers[protocol] = manager
    if manager:
        with service_lock:
            client_states[protocol] = manager.state.copy()
            client_states[protocol]['manager'] = manager
    else:
        with service_lock:
            client_states[protocol] = {'running': False}
...
class BaseClientManager:
    """客户端管理器抽象基类，提供统一的状态管理和生命周期"""

    protocol = 'base'  # 子类必须指定协议名
    ...
    def __init__(self, config: dict):
        ...
        # 延迟调用 _extra_state()，确保子类属性已初始化
        self._init_extra_state()

    def _init_extra_state(self):
        """初始化额外状态字段（在子类 __init__ 完成后调用）"""
        try:
    ...
    def _update_state(self, **kwargs):
        ...
        with self.lock:
            for key, value in kwargs.items():
                self.state[key] = value
            with service_lock:
                client_states[self.protocol] = self.state.copy()
                client_states[self.protocol]['manager'] = self

    def _add_connection(self, conn_id: str, info: dict):
        """添加连接到状态"""
        with self.lock:
            self.state['connections'][conn_id] = info
            with service_lock:
                client_states[self.protocol] = self.state.copy()
                client_states[self.protocol]['manager'] = self
...
class BaseListenerThread(threading.Thread):
    """服务端监听线程抽象基类"""
    ...
    def _update_state(self, **kwargs):
        ...
        with service_lock:
            for key, value in kwargs.items():
                self.state[key] = value
            listener_states[self.protocol] = self.state.copy()
...
class TCPClientManager(BaseClientManager):
    """TCP 客户端管理器，继承自基类并提供心跳检测"""
    protocol = 'tcp'
    ...
    def __init__(self, config):
        ...
        self.connection_target = config.get('connections', 1)
        self.connect_rate = max(config.get('connect_rate', 1), 0.1)
        self.message = config.get('message', '')
        self.send_interval = max(config.get('send_interval', 1), 0.1)
        # MAC 地址配置（用于日志记录）
    ...
    def connect(self):
        """建立 TCP 连接"""
        add_service_log('TCP 客户端', f'准备连接 {self.server_ip}:{self.server_port}，连接数：{self.connection_target}')
        self._update_state(running=True, connections={})
        ...
        add_service_log('TCP 客户端', f'已创建 {self.connection_target} 个连接线程')
    ...
    def start_send(self):
        ...
        add_service_log('TCP 客户端', '开始发送数据')
        ...
        # 更新发送状态，让前端知道正在发送数据
        self._update_state(sending=True)
        return True, '开始发送'
    ...
    def stop_send(self):
        ...
        add_service_log('TCP 客户端', '已停止发送数据')

        # 更新发送状态，让前端知道已停止发送
        self._update_state(sending=False)
        return True, '已停止发送'
        ...
        if conn_id:
            ...
            if sock:
                try:
                    ...
                    pass
            add_service_log('TCP 客户端', f'连接已断开：{conn_id}')
            return True, '连接已断开'
        else:
            self.stop()
            return True, '所有连接已断开'
    ...
    def stop(self):
        """停止所有连接"""
        add_service_log('TCP 客户端', '正在停止所有连接...')
        self.stop_event.set()
        self.send_stop_event.set()

        # 复制字典以避免迭代时修改导致的错误
        contexts_to_stop = list(self.connection_context.values())
        for context in contexts_to_stop:
            context['stop_event'].set()
            sock = context.get('socket')
            if sock:
                try:
        ...
        self.threads.clear()
        self.send_threads.clear()
        self.connection_context.clear()

        self._update_state(running=False, connections={})
        add_service_log('TCP 客户端', '所有连接已停止')
    ...
    def _run_connection(self, conn_id, conn_stop):
        ...
        try:
            ...
            if self.use_local_address and self.local_address:
                try:
                    sock.bind((self.local_address, 0))
                    add_service_log('TCP 客户端', f'绑定本地地址：{self.local_address}', 'info')
                except Exception as e:
                    add_service_log('TCP 客户端', f'绑定本地地址失败：{e}', 'warning')

            mac_info = ''
            if self.src_mac:
                mac_info += f'源 MAC: {self.src_mac}'
            if self.dst_mac:
                if mac_info:
                    mac_info += ', '
                mac_info += f'目的 MAC: {self.dst_mac}'
            if mac_info:
                add_service_log('TCP 客户端', f'MAC 地址配置：{mac_info}', 'info')
            ...
            self._update_connection_status(conn_id, 'connected')

            add_service_log('TCP 客户端', f'连接成功：{self.server_ip}:{self.server_port} (ID: {conn_id[:8]})')
            ...
            while not self.stop_event.is_set() and not conn_stop.is_set():
                try:
                    ...
                    if not data:
                        add_service_log('TCP 客户端', f'对端关闭连接 (ID: {conn_id[:8]})', 'warning')
                        break
                except socket.timeout:
                    continue
                except (ConnectionResetError, ConnectionAbortedError, OSError) as e:
                    add_service_log('TCP 客户端', f'连接异常 (ID: {conn_id[:8]}): {e}', 'error')
                    break

        except Exception as e:
            add_service_log('TCP 客户端', f'连接失败 (ID: {conn_id[:8]}): {e}', 'error')

        finally:
            self._cleanup_connection(conn_id, sock)
        ...
        try:
            while not self.send_stop_event.is_set() and not self.stop_event.is_set():
                try:
                    ...
                    with service_lock:
                        if conn_id in self.state.get('connections', {}):
                            self.state['connections'][conn_id]['bytes_sent'] += len(payload)
                    time.sleep(self.send_interval)
                except (socket.error, OSError) as e:
                    add_service_log('TCP 客户端', f'发送数据异常 (ID: {conn_id[:8]}): {e}', 'error')
                    break
        except Exception as e:
            add_service_log('TCP 客户端', f'发送线程异常 (ID: {conn_id[:8]}): {e}', 'error')

    def _update_connection_status(self, conn_id, status):
        """更新单个连接的状态"""
        with service_lock:
            if conn_id in self.state.get('connections', {}):
                self.state['connections'][conn_id]['status'] = status
                client_states[self.protocol] = self.state.copy()

    def _cleanup_connection(self, conn_id, sock):
        """清理单个连接"""
        if sock:
            try:
                sock.close()
        ...
        if not self.connection_context:
            add_service_log('TCP 客户端', '所有连接已断开，状态已更新为未连接')




class UDPClientManager(BaseClientManager):
    """UDP 客户端管理器，继承自基类"""
    protocol = 'udp'
    ...
    def connect(self):
        """开始 UDP 发送"""
        add_service_log('UDP 客户端', f"开始发送到 {self.server_ip}:{self.server_port}")
        self._update_state(running=True, connections={})
        ...
        add_service_log('UDP 客户端', f'已创建 {self.connection_target} 个发送线程')

    def stop(self):
        """停止发送"""
        add_service_log('UDP 客户端', '正在停止发送...')
        self.stop_event.set()
        for worker in self.workers:
            worker.join(timeout=0.5)
        self.workers.clear()
        add_service_log('UDP 客户端', '发送已停止')

    def disconnect(self, conn_id=None):
        """断开连接（UDP 无连接，只是停止发送）"""
        if conn_id:
            return True, 'UDP 无连接状态'
        else:
            self.stop()
            return True, '已停止发送'
...
class FTPClientManager(BaseClientManager):
    """FTP 客户端管理器，继承自基类"""
    protocol = 'ftp'
    ...
    def connect(self):
        ...
        try:
            self.ftp = ftplib.FTP()
            # 尝试连接（多次数，逐步增加超时）
            for timeout in [3, 8, 12]:
                try:
                    self.ftp.connect(self.server_ip, self.server_port, timeout=timeout)
                    break
                except:
                    continue
            ...
            self.connected = True
            self._update_state(running=True, current_dir=self.current_dir)
            add_service_log('FTP 客户端', f'已连接 {self.server_ip}:{self.server_port} (被动模式)')
            return True, '连接成功'
        except Exception as e:
            add_service_log('FTP 客户端', f'连接失败：{e}', 'error')
            self._cleanup()
            return False, str(e)
    ...
    def list_files(self):
        ...
        try:
            ...
            # 使用 nlst 获取纯文件名列表（无权限、时间等信息，避免编码问题）
            # 尝试多种编码方式处理 Windows FTP 服务器的中文文件名
            files = None
            ...
            for encoding in encodings:
                try:
                    ...
                    files = self.ftp.nlst()
                    add_service_log('FTP 客户端', f'nlst 成功，编码：{encoding}, 文件数：{len(files) if files else 0}')
                    break
                    ...
                    # 其他异常直接返回
                    add_service_log('FTP 客户端', f'nlst 失败 ({encoding}): {type(e).__name__}: {e}', 'error')
                    return False, f'nlst 失败 ({encoding}): {type(e).__name__}: {e}'

            if files is None:
                add_service_log('FTP 客户端', f'所有编码尝试失败：{last_error}', 'error')
                return False, f'所有编码尝试失败：{last_error}'

            # 确保所有文件名都是字符串（处理可能的 bytes 返回）
            cleaned_files = []
            for i, f in enumerate(files):
                if isinstance(f, bytes):
                    ...
                    for enc in encodings:
                        try:
                            cleaned_files.append(f.decode(enc))
                            decoded = True
                            break
                        except:
                            continue
                    if not decoded:
                        cleaned_files.append(f.decode('latin-1', errors='replace'))
                else:
                    cleaned_files.append(str(f))

            add_service_log('FTP 客户端', f'文件列表已清理，共 {len(cleaned_files)} 个文件')

            # 格式化为类似 UNIX ls -l 的输出（简化版）
            file_list_str = '\n'.join(cleaned_files)
            ...
            add_service_log('FTP 客户端', f'准备调用 _update_state')
            self._update_state(file_list=file_list_str, current_dir=current_dir)
            add_service_log('FTP 客户端', f'_update_state 完成，准备返回')
            return True, {'files': file_list_str, 'current_dir': current_dir}

        except Exception as e:
            # 捕获所有未处理的异常
            add_service_log('FTP 客户端', f'list_files 未处理异常：{type(e).__name__}: {e}', 'error')
            return False, f'list_files 错误：{type(e).__name__}: {e}'
    ...
    def upload_file(self, filename, content):
        ...
        try:
            ...
            add_service_log('FTP 客户端', f'上传完成：{filename}')
            ...
            add_service_log('FTP 客户端', f'上传失败：{e}', 'error')
    ...
    def download_file(self, filename):
        ...
        try:
            ...
            add_service_log('FTP 客户端', f'下载完成：{filename} (大小：{file_size or len(file_data)} 字节)')
            return True, {
                'filename': filename,
            ...
            add_service_log('FTP 客户端', f'下载失败：{e}', 'error')
            return False, str(e)
...
class MailListenerThread(threading.Thread):
    def __init__(self, state):
        ...
        self.domain = state.get('domain', 'autotest.com')
        ...
        # 邮件存储配置
        self.mail_storage_dir = os.path.join(os.getcwd(), 'mail_storage')
        ...
        print(f'*** 邮件存储目录: {self.mail_storage_dir} ***')
        print(f'*** 当前工作目录: {os.getcwd()} ***')
        add_service_log('邮件服务器', f'*** 邮件存储目录: {self.mail_storage_dir} ***')
        add_service_log('邮件服务器', f'*** 当前工作目录: {os.getcwd()} ***')
        self._init_mail_storage()
        self._init_db()  # 初始化 SQLite 数据库
        ...
        print(f'*** 接收到的账户数量: {len(self.accounts)} ***')
    ...
    def _save_mail(self, sender, recipients, subject, content, mail_data=None):
        """保存邮件到收件人的邮箱"""
        try:
            print(f'*** _save_mail 被调用！sender={sender}, recipients={recipients}, subject={subject} ***')
            timestamp = time.strftime('%Y%m%d_%H%M%S')
            mail_id = f"{timestamp}_{hash(content) % 10000:04d}"
            print(f'*** 生成邮件ID: {mail_id} ***')
            
            # 为每个收件人保存邮件
            print(f'*** 开始为 {len(recipients)} 个收件人保存邮件 ***')
            for recipient in recipients:
                # 提取用户名（去掉@domain部分）
                username = recipient.split('@')[0] if '@' in recipient else recipient
                print(f'*** 处理收件人: {recipient} -> 用户名: {username} ***')
                user_mail_dir = os.path.join(self.mail_storage_dir, username, 'INBOX')
                print(f'*** 用户邮件目录: {user_mail_dir} ***')
...
{content}"""
                ...
                # 如果有原始邮件数据，使用原始数据
                if mail_data:
                    print(f'*** 使用原始邮件数据，长度: {len(mail_data)} ***')
                ...
                add_service_log('邮件服务器', f'*** 邮件文件路径: {mail_file} ***')
                add_service_log('邮件服务器', f'*** 用户目录: {user_mail_dir} ***')
            
            print(f'*** _save_mail 完成，共处理 {len(recipients)} 个收件人 ***')
                    
        except Exception as e:
            print(f'*** _save_mail 异常: {str(e)} ***')
            import traceback
            print(f'*** _save_mail 异常堆栈: {traceback.format_exc()} ***')
                ...
                try:
                    ...
                    if cmd == 'USER':
                        ...
                        try:
                            ...
                            if 1 <= msg_num <= len(user_mails) and (msg_num - 1) not in deleted_mails:
                                mail = user_mails[msg_num - 1]
                                mail_content = f"""From: {mail.get('from', 'unknown')}
To: {mail.get('to', 'unknown')}
...
{mail.get('body', '')}"""
                                ...
                                client_socket.send(mail_content.encode() + b'\r\n.\r\n')
                                add_service_log('邮件服务器', f'📬 POP3发送邮件: {msg_num}')
                            else:
                                client_socket.send(b'-ERR No such message\r\n')
                        except ValueError:
                            client_socket.send(b'-ERR Invalid message number\r\n')
    ...
    def _get_user_mails(self, username, folder='INBOX'):
        ...
        try:
            ...
            # 按日期排序（最新的在前）
            mails.sort(key=lambda x: x.get('date', ''), reverse=True)
            return mails
    ...
    def _run_smtp_server(self, host):
        ...
        try:
            ...
            print(f'*** SMTP 服务器启动成功：{host}:{self.smtp_port} ***')
    ...
    def _create_default_test_account(self):
        ...
        try:
            ...
            add_service_log('邮件服务器', f'已创建默认测试账户: test@autotest.com')
            add_service_log('邮件服务器', f'密码: "{test_password}" (长度: {len(test_password)})')
            add_service_log('邮件服务器', f'密码字节: {test_password.encode("utf-8")}')
            add_service_log('邮件服务器', f'账户文件保存到: {self.accounts_file}')
            
            # 立即验证创建的账户
            accounts = self._load_accounts()
            if 'test' in accounts:
                stored_password = accounts['test'].get('password', '')
                add_service_log('邮件服务器', f'验证创建结果: 存储密码="{stored_password}" (长度: {len(stored_password)})')
                add_service_log('邮件服务器', f'验证创建结果: 密码匹配={stored_password == test_password}')
            
        except Exception as e:
            add_service_log('邮件服务器', f'创建默认测试账户失败: {e}', 'error')

    def _get_user_mails(self, conn_id):
        """获取用户的邮件列表"""
        try:
            # 从连接状态获取用户名
            with service_lock:
            ...
            print(f'*** _get_user_mails: 用户 {username} 收件箱中有 {len(mail_files)} 封邮件 ***')
            print(f'*** _get_user_mails: 查找目录: {user_inbox} ***')
            print(f'*** _get_user_mails: 目录存在: {os.path.exists(user_inbox)} ***')
            if os.path.exists(user_inbox):
                all_files = os.listdir(user_inbox)
                print(f'*** _get_user_mails: 目录中所有文件: {all_files} ***')
            
            add_service_log('邮件服务器', f'*** 用户 {username} 收件箱中有 {len(mail_files)} 封邮件 ***')
            add_service_log('邮件服务器', f'*** 查找目录: {user_inbox} ***')
            add_service_log('邮件服务器', f'*** 目录存在: {os.path.exists(user_inbox)} ***')
            if mail_files:
                for i, (mtime, path, filename) in enumerate(mail_files[:3]):  # 只显示前3个
                    add_service_log('邮件服务器', f'*** 邮件{i+1}: {filename} ***')
                    print(f'*** _get_user_mails: 邮件{i+1}: {filename} -> {path} ***')
            # 限制返回最近的10封邮件
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    # SELECT命令处理
                        ...
                        # 获取当前用户的邮件数量
                        mail_list = self._get_user_mails(conn_id)
    ...
    def _handle_pop3_client(self, client_socket, addr, conn_id):
        """处理POP3客户端连接"""
        try:
            add_service_log('邮件服务器', f'POP3客户端连接: {addr[0]}:{addr[1]}')
                ...
                try:
                    ...
                    if command.upper().startswith('USER'):
                        ...
                        if len(parts) >= 2:
                            ...
                            client_socket.send(b'-ERR USER command error\r\n')
                    elif command.upper().startswith('PASS'):
                        parts = command.split()
                        if len(parts) >= 2 and current_user:
                            password = parts[1]
                            is_valid, email = self._authenticate_user(current_user, password)
                            if is_valid:
                                ...
                                with service_lock:
                                    if conn_id in self.state['connections']:
                                        self.state['connections'][conn_id]['authenticated'] = True
                                        self.state['connections'][conn_id]['email'] = email
                                add_service_log('邮件服务器', f'POP3用户登录成功: {email}')
                            else:
                                client_socket.send(b'-ERR Password incorrect\r\n')
                                add_service_log('邮件服务器', f'POP3用户登录失败: {current_user}')
                        else:
                            client_socket.send(b'-ERR PASS command error\r\n')
                    elif command.upper().startswith('STAT'):
                        client_socket.send(b'+OK 0 0\r\n')  # 0 messages, 0 octets
                    ...
                    add_service_log('邮件服务器', f'POP3处理错误: {e}', 'error')
                    break
    ...
    with service_lock:
        ...
        if protocol == 'http':
            ...
            if directory:
                ...
                # 默认使用脚本目录下的http目录
                state['directory'] = os.path.join(script_dir, 'http')
            # 确保目录存在
            try:
                os.makedirs(state['directory'], exist_ok=True)
            except Exception as e:
...
def start_tcp_client(config):
    ...
    # 停止旧管理器（在锁外）
    old_manager = _get_client_manager('tcp')
    if old_manager:
        add_service_log('TCP 客户端', '检测到旧连接，正在停止...', 'warning')
        old_manager.stop()
        for thread in old_manager.threads:
            thread.join(timeout=0.5)
        add_service_log('TCP 客户端', '旧连接已停止', 'info')

    # 创建新管理器
    manager = TCPClientManager(config)
    _set_client_manager('tcp', manager)
    manager.connect()
    time.sleep(0.5)  # 等待连接建立
    manager.start_send()

    add_service_log('TCP 客户端', f'TCP 客户端已启动，耗时 {time.time()-_start_time:.2f}s', 'info')
    return True, {'message': 'TCP 客户端已启动'}
...
def connect_tcp_client(config):
    ...
    add_service_log('TCP 客户端', f'TCP 客户端已连接，耗时 {time.time()-_start_time:.2f}s', 'info')
    return True, {'message': 'TCP 客户端已连接'}



def start_tcp_send(config):
    """开始发送数据"""
    manager = _get_client_manager('tcp')
    if not manager:
        return False, 'TCP 客户端未连接'
    # 更新发送内容
...
def stop_tcp_client():
    ...
    if not manager:
        return False, 'TCP 客户端未运行'
    manager.stop()
    _set_client_manager('tcp', None)
    return True, {'message': 'TCP 客户端已停止'}
...
def stop_udp_client():
    ...
    if manager:
        manager.stop()
        # manager.join() not needed - UDPClientManager is not a Thread
    with service_lock:
...
def connect_ftp_client(config):
    ...
    if not server_ip or server_port <= 0:
        return False, 'FTP 服务器地址或端口无效'

    # 停止旧管理器
    old_manager = _get_client_manager('ftp')
    if old_manager:
        add_service_log('FTP 客户端', '检测到旧连接，正在停止...', 'warning')
        old_manager.disconnect()

    # 创建新管理器并连接
    manager = FTPClientManager(config)
    _set_client_manager('ftp', manager)
    ...
    if success:
        ...
        def get_file_list_async():
            ...
            try:
                ...
                add_service_log('FTP 客户端', f'获取文件列表失败：{e}', 'error')
        ...
        return True, {
            ...
            'file_list': manager.state.get('file_list', '')
        }
    else:
        _set_client_manager('ftp', None)
        return False, {'error': message}


def disconnect_ftp_client(data=None):
    """断开 FTP 连接"""
    manager = _get_client_manager('ftp')
    if not manager or not manager.connected:
        return False, 'FTP 客户端未连接'
    ...
    try:
        ...
        return True, {'message': 'FTP 客户端已断开'}
    except Exception as e:
        return False, f'断开连接失败：{e}' 


def list_ftp_files(data=None):
    """获取 FTP 文件列表"""
    manager = _get_client_manager('ftp')
    if not manager or not manager.connected:
        return False, 'FTP 客户端未连接'
    ...
    try:
        ...
        add_service_log('FTP 客户端', f'list_files 返回：{type(result)} - {result}', 'info')

        if isinstance(result, tuple) and len(result) == 2:
            success, data_result = result
            if success:
                # data_result 是 {'files': file_list, 'current_dir': ...}
                # 直接返回 file_list 字符串
                file_list = data_result.get('files', '')
                add_service_log('FTP 客户端', f'提取文件列表：{type(file_list)} - {file_list[:100] if file_list else "(空)"}', 'info')
                return True, {'files': file_list, 'file_list': file_list}
            else:
                add_service_log('FTP 客户端', f'list_files 返回失败：{data_result}', 'error')
                return result
        else:
            add_service_log('FTP 客户端', f'list_files 返回非预期格式：{result}', 'warning')
            return True, {'files': str(result), 'file_list': str(result)}
    except Exception as e:
        add_service_log('FTP 客户端', f'list_ftp_files 异常：{e}', 'error')
        import traceback
        add_service_log('FTP 客户端', f'Stack trace: {traceback.format_exc()}', 'error')
        return False, str(e)


def upload_ftp_file(filename, content=None, local_file_path=None, is_binary=False):
    """上传文件到 FTP 服务器"""
    manager = _get_client_manager('ftp')
    if not manager or not manager.connected:
        return False, 'FTP 客户端未连接'
    ...
    if isinstance(content, str):
        try:
            ...
            # 如果不是 base64，则作为普通字符串处理
            content = content.encode('utf-8')

    return manager.upload_file(filename, content)
...
class HTTPClientManager(BaseClientManager):
    """HTTP 客户端管理器，继承自基类"""
    protocol = 'http'
        ...
        try:
            ...
            try:
                with urllib.request.urlopen(req, timeout=5) as response:
                    ...
                    add_service_log('HTTP 客户端', f'连接成功：{self.server_ip}:{self.server_port}')
            ...
            add_service_log('HTTP 客户端', f'连接失败：{e}', 'error')
            return False, str(e)
    ...
    def disconnect(self):
        ...
        self.connected = False
        self._update_state(running=False)
        add_service_log('HTTP 客户端', '连接已断开')
        return True, '连接已断开'
    ...
    def list_files(self):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                self._update_state(file_list=files)
                add_service_log('HTTP 客户端', f'获取文件列表成功：{len(files)} 项')
                return True, files
        except Exception as e:
            add_service_log('HTTP 客户端', f'获取文件列表失败：{e}', 'error')
            return False, str(e)
    ...
    def download_file(self, filename):
        ...
        try:
            import urllib.request
            import urllib.error
            url = f'http://{self.server_ip}:{self.server_port}/{filename}'
            req = urllib.request.Request(url)
            req.add_header('User-Agent', 'HTTPClient/1.0')
            with urllib.request.urlopen(req, timeout=30) as response:
                data = response.read()
                actual_size = len(data)
                add_service_log('HTTP 客户端', f'下载完成：{filename} (大小：{actual_size} 字节)')
            ...
            if e.code == 404:
                add_service_log('HTTP 客户端', f'文件不存在：{filename}', 'error')
                return False, '文件不存在'
            else:
                return False, str(e)
        except Exception as e:
            add_service_log('HTTP 客户端', f'下载失败：{e}', 'error')
            return False, str(e)
...
def connect_http_client(config):
    ...
    if not server_ip or server_port <= 0:
        return False, 'HTTP 服务器地址或端口无效'

    # 停止旧管理器
    old_manager = _get_client_manager('http')
    if old_manager:
        add_service_log('HTTP 客户端', '检测到旧连接，正在停止...', 'warning')
        old_manager.disconnect()

    # 创建新管理器并连接
    manager = HTTPClientManager(config)
    _set_client_manager('http', manager)
    success, message = manager.connect()
    ...
    if success:
        ...
        _set_client_manager('http', None)
        return False, {'error': message}


def disconnect_http_client():
    """断开 HTTP 连接"""
    manager = _get_client_manager('http')
    if not manager:
        return False, 'HTTP 客户端未连接'
    manager.disconnect()
    _set_client_manager('http', None)
    return True, {'message': 'HTTP 连接已断开'}
                    ...
                    for mail_id in recent_ids:
                        try:
                            ...
                            if typ == 'OK':
                                ...
                                date_raw = msg.get('Date', '未知')
                                
                                # 格式化日期为中文格式
                                date_formatted = format_email_date(date_raw)
...
def send_mail_to_local_storage(mail_data):
    ...
    try:
        ...
        # 邮件存储目录
        mail_storage_dir = os.path.join(os.getcwd(), 'mail_storage')
...
{content}"""
            ...
            add_service_log('邮件客户端', f'*** 邮件已保存到: {user_inbox}/{mail_id}.eml ***', 'info')
    ...
    try:
        ...
        for protocol in client_states.keys():
            ...
            if state and state.get('running'):
                ...
                summary = {'running': False, 'sending': False}
...
def api_services_listener():
    try:
        ...
        print(f'*** 协议: {protocol}, 操作: {action}, 主机: {host}, 端口: {port} ***')
        ...
        if action == 'start':
            ...
            if protocol == 'ftp':
                ...
                print(f'*** 邮件参数: {kwargs} ***')
                print(f'*** 接收到的账户数据: {accounts_data} ***')
                print(f'*** 账户数据类型: {type(accounts_data)}, 长度: {len(accounts_data)} ***')
            
            print(f'*** 开始启动 {protocol} 服务 ***')
            success, result = start_listener(protocol, host, port, **kwargs)
            print(f'*** 启动结果: success={success}, result={result} ***')
        elif action == 'stop':
            print(f'*** 开始停止 {protocol} 服务 ***')
            success, result = stop_listener(protocol)
            print(f'*** 停止结果: success={success}, result={result} ***')
        else:
...
def api_mail_recent():
    ...
    try:
        ...
        # 临时方案：直接查询数据库
        import sqlite3
        db_path = os.path.join(os.getcwd(), 'mail_storage', 'mails.db')
...
CLIENT_HANDLERS = {
    'tcp': {
        'start': lambda d: start_tcp_client(d),
        'connect': lambda d: connect_tcp_client(d),
        'start_send': lambda d: start_tcp_send(d),
        'stop_send': lambda d: stop_tcp_send(),
        'stop': lambda d: stop_tcp_client(),
        'disconnect': lambda d: disconnect_tcp_connection(d.get('connection_id')),
    },
    'udp': {
        'start': lambda d: start_udp_client(d),
        'stop': lambda d: stop_udp_client(),
    },
    'ftp': {
        'start': lambda d: start_ftp_client(d),
        'connect': lambda d: connect_ftp_client(d),
        'disconnect': lambda d: disconnect_ftp_client(),
        'list': lambda d: list_ftp_files(),
        'upload': lambda d: upload_ftp_file(
            d.get('filename', ''),
            d.get('content', ''),
            d.get('local_file_path', ''),
            d.get('is_binary', False)
        ),
        'get_local_files': lambda d: get_local_file_list(d.get('directory', '')),
        'download': lambda d: download_ftp_file(d.get('filename', '')),
        'stop': lambda d: stop_ftp_client(),
    },
    'http': {
        'connect': lambda d: connect_http_client(d),
        'disconnect': lambda d: disconnect_http_client(),
        'list': lambda d: list_http_files(),
        'download': lambda d: download_http_file(d.get('filename', '')),
    },
    'mail': {
        'test_connection': lambda d: test_mail_connection(
            d.get('type', 'smtp'),
            d.get('config', {})
        ),
        'send_mail': lambda d: send_mail_via_smtp(
            d.get('smtp_config', {}),
            d.get('mail_data', {}),
            d.get('source_ip', '')
        ),
        'get_inbox': lambda d: get_inbox_mails(
            d.get('receive_config', {}),
            d.get('source_ip', '')
        ),
    },
    'network': {
        'ping': lambda d: test_network_connectivity(
            d.get('server', ''),
            d.get('port', 0)
        ),
    },
...
def api_services_client():
    ...
    add_service_log('API 调试', 'api_services_client 函数被调用', 'info')
    ...
    try:
        ...
        add_service_log('API 调试', f'收到客户端请求：protocol={protocol}, action={action}', 'info')

        # 协议路由
        if protocol not in CLIENT_HANDLERS:
            return jsonify({'success': False, 'error': f'不支持的协议：{protocol}'}), 400

        protocol_handlers = CLIENT_HANDLERS[protocol]
        if action not in protocol_handlers:
            return jsonify({'success': False, 'error': f'不支持的操作：{action}'}), 400

        # 调用处理函数
        handler = protocol_handlers[action]
        result = handler(data)
        ...
        if isinstance(result, (list, tuple)) and len(result) == 2:
            ...
            success, data_result = True, result

        # 格式化响应
        status_code = 200 if success else 400

        if isinstance(data_result, dict):
            return jsonify({'success': success, **data_result}), status_code
        else:
            return jsonify({'success': success, 'message': str(data_result)}), status_code

    except Exception as e:
        import traceback
        error_msg = f'api_services_client 异常：{e}\n{traceback.format_exc()}'
        add_service_log('API 错误', error_msg, 'error')
...
def api_services_status():
    try:
        ...
        # 调试日志
        print(f"[DEBUG] listener_states keys: {list(listener_states.keys())}")
        for proto, state in listener_states.items():
            print(f"[DEBUG] {proto} state: running={state.get('running') if state else None}")
        ...
        for protocol in ['tcp', 'udp', 'ftp', 'http', 'mail']:
            ...
            print(f"[DEBUG] api_services_status - protocol={protocol}, is_running={is_running}, state={state}")
            ...
            if is_running and protocol == 'mail':
                ...
                if thread and hasattr(thread, 'is_alive'):
                    try:
                        ...
                        print(f"[DEBUG] mail status check error: {e}")
                        listeners_summary[protocol] = {'running': False}
                else:
                    listeners_summary[protocol] = {'running': False}
            elif is_running:
        ...
        for protocol in ['tcp', 'udp', 'ftp', 'http', 'mail']:
            ...
            if is_running:
                ...
                clients_summary[protocol] = {'running': False, 'sending': False}

        print(f"[DEBUG] listeners_summary: {listeners_summary}")
        print(f"[DEBUG] clients_summary: {clients_summary}")
        ...
        print(f"[ERROR] api_services_status exception: {e}")
        import traceback
