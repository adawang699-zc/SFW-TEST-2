...
FRAME_ID_MAP = {"HELLO": 0xFEFC, "GETORSET": 0xFEFD, "IDENT_REQ": 0xFEFF, "IDENT_RES": 0xFEFE}
SERVICE_ID_MAP = {"GET": 0x03, "SET": 0x04, "IDENTIFY": 0x05, "HELLO": 0x00, "RESERV": 0x06, "MANU": 0x07}
SERVICE_TYPE_MAP = {"request": 0x00, "response_success": 0x01}
OPTION_MAP = {
    "IP": 0x01, "DEVICE": 0x02, "DHCP": 0x03, "RESERVED": 0x04, "CONTROL": 0x05,
    "DEVICEINITIATIVE": 0x06, "NME_PARAMS": 0x07,
    "MANUF_X80": 0x80, "MANUF_X81": 0x81, "MANUF_X82": 0x82, "MANUF_X83": 0x83,
    "MANUF_X84": 0x84, "MANUF_X85": 0x85, "MANUF_X86": 0x86, "ALLSECECTOR": 0xFF,
}
OPTION_SUBOPTIONS = {
    0x01: [(0x01, "MAC address"), (0x02, "IP parameter"), (0x03, "Full IP suite")],
    0x02: [(0x01, "Type of Station"), (0x02, "Name of Station"), (0x03, "Device ID"), (0x04, "Device Role"), (0x05, "Device Options"), (0x06, "Alias Name"), (0x07, "Device Instance"), (0x08, "OEM Device ID")],
    0x03: [(0x0C, "Host name"), (0x2B, "Vendor specific"), (0x36, "Server identifier"), (0x37, "Parameter request list"), (0x3C, "Class identifier"), (0x3D, "DHCP client identifier"), (0x51, "FQDN"), (0x61, "UUID/GUID-based Client"), (0xFF, "Control DHCP")],
    0x04: [(0x01, "Reserved 0x01")],
    0x05: [(0x01, "Start Transaction"), (0x02, "End Transaction"), (0x03, "Signal"), (0x04, "Response"), (0x05, "Reset Factory Settings"), (0x06, "Reset to Factory")],
    0x06: [(0x01, "Device Initiative")],
    0x07: [(0x01, "NME Domain Name"), (0x02, "NME Manager"), (0x03, "NME Paramater UUID"), (0x04, "NME Agent"), (0x05, "CIM Interface")],
    0x80: [(0x01, "Manufacturer 0x80")], 0x81: [(0x01, "Manufacturer 0x81")], 0x82: [(0x01, "Manufacturer 0x82")],
    0x83: [(0x01, "Manufacturer 0x83")], 0x84: [(0x01, "Manufacturer 0x84")], 0x85: [(0x01, "Manufacturer 0x85")], 0x86: [(0x01, "Manufacturer 0x86")],
...
def build_and_send_dcp_request(iface, frame_id, service_id, service_type, option, suboption, dst_mac="01:0e:cf:00:00:00", src_mac="00:11:22:33:44:55", xid=0x00010203):
    multi_blocks = struct.pack("!BB", option, suboption)
    dcp_data_length = len(multi_blocks)
    dcp_core = struct.pack("!H BB I H H", frame_id & 0xFFFF, service_id & 0xFF, service_type & 0xFF, xid & 0xFFFFFFFF, 0x0000, dcp_data_length) + multi_blocks
...
class DcpSenderService:
    ...
    def set_config(self, config):
        ...
        if isinstance(subs, (list, tuple)) and len(subs) > 0:
            self.suboptions = [int(s) & 0xFF for s in subs if s is not None]
    ...
    def start(self):
        ...
        try:
            ...
            self.thread = threading.Thread(target=self._send_loop, daemon=True, args=(frame_id, service_id, service_type, option_val))
            self.thread.start()
            return True
    ...
    def _send_loop(self, frame_id, service_id, service_type, option_val):
        idx, subopts, n = 0, list(self.suboptions), len(self.suboptions)
        while self.is_running and n:
            suboption_val = subopts[idx % n]
            idx += 1
            try:
                build_and_send_dcp_request(self.iface, frame_id, service_id, service_type, option_val, suboption_val, dst_mac=self.dst_mac, src_mac=self.src_mac)
                self.packet_count += 1
                if self.callback:
